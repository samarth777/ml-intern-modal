...
async def main():
    ...
    tool_router = ToolRouter(
        ...
        hf_token=hf_token,
        local_mode=True,
        compute_backend=getattr(config, "compute_backend", "modal"),
    )
...
def _run_copilot_command(sub: str) -> None:
    """Implement `ml-intern copilot {login,logout,status}`.

    Imported lazily so that ``import agent.main`` does not pull in the
    ``requests``-based provider module unless the user actually invokes
    the subcommand.
    """
    import json as _json
    import webbrowser

    from agent.providers import copilot

    if sub == "logout":
        removed = copilot.clear_credentials()
    ...
    # ── login ──
    device = copilot.request_device_code()
    print(f"\nOpen this URL and enter the code below:")
    print(f"  URL:  {device.verification_uri}")
    print(f"  Code: {device.user_code}\n")
    try:
        webbrowser.open(device.verification_uri)
    except Exception:
        pass  # headless environment — user copy-pastes the URL manually.

    print("Waiting for authorization (Ctrl-C to cancel)...")
    try:
        oauth_token = copilot.poll_for_oauth_token(device)
    except (TimeoutError, RuntimeError) as e:
        print(f"Login failed: {e}")
        sys.exit(1)

    creds = copilot.CopilotCredentials(oauth_token=oauth_token)
    copilot.save_credentials(creds)
    # Eagerly verify Copilot access so failure (e.g. no Copilot subscription)
    # surfaces here instead of at the next LLM call.
    try:
        copilot.verify_copilot_access(oauth_token)
    except RuntimeError as e:
        print(f"Logged in, but Copilot access check failed: {e}")
        sys.exit(1)
    print(f"Logged in. Credentials cached at {copilot._cache_path()}")
    print("Use a model like `copilot/claude-sonnet-4` in your config.")
...
def cli():
    ...
    # Subcommands (e.g. `ml-intern copilot login`) are handled before the main
    # parser so they don't conflict with the optional positional ``prompt``.
    import sys as _sys

    if len(_sys.argv) >= 2 and _sys.argv[1] == "copilot":
        sub = _sys.argv[2] if len(_sys.argv) >= 3 else "status"
        if sub not in {"login", "logout", "status"}:
            print(f"Unknown subcommand: copilot {sub}")
            print("Usage: ml-intern copilot {login|logout|status}")
            _sys.exit(2)
        _run_copilot_command(sub)
        return
