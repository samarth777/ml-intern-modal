...
class ToolRouter:
    ...
    def __init__(
        ...
        for tool in create_builtin_tools(
            local_mode=local_mode, compute_backend=compute_backend
        ):
...
def create_builtin_tools(
    local_mode: bool = False, compute_backend: str = "hf"
) -> list[ToolSpec]:
    """Create built-in tool specifications.

    Args:
        local_mode: When True, use local subprocess tools (CLI mode).
        compute_backend: "hf" or "modal" — selects which sandbox + jobs
            implementation gets registered. Ignored when local_mode is True.
    """
    ...
    tools = [
        ...
        # NOTE: the jobs tool (hf_jobs) is registered below by the
        # compute_backend registry — its impl differs between hf / modal.
    ...
    # Sandbox + jobs tools (highest priority, prepended). Resolved by the
    # compute_backend registry so swapping "hf" ↔ "modal" only changes the
    # underlying implementation, not the agent-facing tool names/schemas.
    from agent.tools.compute_backend import get_compute_tools

    tools = get_compute_tools(compute_backend, local_mode=local_mode) + tools
