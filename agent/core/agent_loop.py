...
def _resolve_copilot_params(model_name: str) -> dict:
    """Build LiteLLM kwargs for ``copilot/<model>`` model strings.

    Routes through GitHub Copilot's OpenAI-compatible chat endpoint using
    the device-flow OAuth token cached by ``ml-intern copilot login``.
    The session token is short-lived (~25 min); :func:`copilot.get_session_token`
    ...
    return {
        ...
        "api_base": copilot.COPILOT_API_BASE,
        "api_key": copilot.get_oauth_token(),
        "extra_headers": copilot.EDITOR_HEADERS,
    }
...
def _resolve_hf_router_params(
    ...
    """
    if model_name.startswith("copilot/"):
        return _resolve_copilot_params(model_name)
        ...
        while max_iterations == -1 or iteration < max_iterations:
            ...
            try:
                ...
                if non_approval_tools:
                    ...
                    parsed_tools: list[tuple[ToolCall, str, dict, bool, str]] = []
