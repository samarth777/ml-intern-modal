...
def _public_host() -> str | None:
    """Return the externally-reachable host (no scheme) when running in a
    known hosting environment.

    Supports HF Spaces (``SPACE_HOST``) and Modal (``MODAL_PUBLIC_URL``,
    or fall back to constructing one from ``MODAL_WORKSPACE`` /
    ``MODAL_ENVIRONMENT`` if explicitly set via ``PUBLIC_HOST``).
    """
    # Explicit override always wins.
    public_host = os.environ.get("PUBLIC_HOST")
    if public_host:
        return public_host.removeprefix("https://").removeprefix("http://")
    # HF Spaces.
    space_host = os.environ.get("SPACE_HOST")
    if space_host:
        return space_host
    # Modal exposes the deployed URL via MODAL_PUBLIC_URL on web endpoints.
    modal_url = os.environ.get("MODAL_PUBLIC_URL")
    if modal_url:
        return modal_url.removeprefix("https://").removeprefix("http://")
    return None
...
def get_redirect_uri(request: Request) -> str:
    ...
    # Otherwise construct from the inbound request (local dev).
...
async def oauth_callback(
    ...
    is_production = bool(_public_host())
